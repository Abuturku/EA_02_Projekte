...
public class Selection implements ISelection {
	...
	private IChromosome[] getParentsWithTournamentSelection(IPopulation fatherPartOfPopulation,
			IPopulation motherPartOfPopulation) {
		IChromosome[] parents = new Chromosome[2];
	...
	private IChromosome[] getParentsWithRouletteWheelSelection(IPopulation fatherPartOfPopulation,
			IPopulation motherPartOfPopulation) {
		IChromosome[] parents = new Chromosome[2];
	...
	private Range getRangeFromChromosomeInPopulation(double count, double sumPopulationFitness,
			IChromosome chromosome) {
		double selectionProbability = getSelectionProbabilityFromChromosomeInPopulation(sumPopulationFitness,
				chromosome);
		return new Range(count + 0.00001, count + selectionProbability);
	}

	private double getSelectionProbabilityFromChromosomeInPopulation(double sumPopulationFitness,
			IChromosome chromosome) {
	...
	private double getRandomValueBetweenNullAndOne() {
		double randomValue = randomGenerator.nextDouble(true, false);// 1
																		// ausgeschlossen
																		// 0
																		// eingeschlossen
